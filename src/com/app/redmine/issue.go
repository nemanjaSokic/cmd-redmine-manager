...
func (c *Client) CreateIssue(issue Issue) (*Issue, error) {
	...
	req, err := http.NewRequest("POST", c.endpoint+"/issues.json?", strings.NewReader(string(s)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) UpdateIssue(issue Issue) error {
	...
	req, err := http.NewRequest("PUT", c.endpoint+"/issues/"+strconv.Itoa(issue.Id)+".json?", strings.NewReader(string(s)))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
...
func (c *Client) DeleteIssue(id int) error {
	req, err := http.NewRequest("DELETE", c.endpoint+"/issues/"+strconv.Itoa(id)+".json?", strings.NewReader(""))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
