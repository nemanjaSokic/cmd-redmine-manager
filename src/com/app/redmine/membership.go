...
func (c *Client) Memberships(projectId int) ([]Membership, error) {
	res, err := c.Get(c.endpoint + "/projects/" + strconv.Itoa(projectId) + "/memberships.json" + c.getPaginationClause())
	if err != nil {
		return nil, err
	}
...
func (c *Client) Membership(id int) (*Membership, error) {
	res, err := c.Get(c.endpoint + "/memberships/" + strconv.Itoa(id) + ".json")
	if err != nil {
		return nil, err
	}
	defer res.Body.Close()
...
func (c *Client) CreateMembership(membership Membership) (*Membership, error) {
	...
	req, err := http.NewRequest("POST", c.endpoint+"/memberships.json", strings.NewReader(string(s)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) UpdateMembership(membership Membership) error {
	...
	req, err := http.NewRequest("PUT", c.endpoint+"/memberships/"+strconv.Itoa(membership.Id)+".json", strings.NewReader(string(s)))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
...
func (c *Client) DeleteMembership(id int) error {
	req, err := http.NewRequest("DELETE", c.endpoint+"/memberships/"+strconv.Itoa(id)+".json", strings.NewReader(""))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
